...
describe('list_people', () => {
  ...
  .command(['list_people', '--count', '3'])
  .it('runs list_people --count 3', ctx => {

    // Must be in the returned list
    expect(ctx.stdout).to.contain('russellwhyte');
    expect(ctx.stdout).to.contain('ronaldmundy');
    expect(ctx.stdout).to.contain('scottketchum');

    // Not in the list of data
